...
module.exports = (req, res) => {
    addsignalsubSchema.findById(req.body.id, (err, signalSubModel) => {
        ...
        if (err) { return res.json({ message: err, status: 0 }) }

        if (signalSubModel == null) { return res.json({ message: 'this is not bitrogen subscription model', status: 0 }) }

        const months = signalSubModel.months
        var amount = signalSubModel.price * signalSubModel.dollar
        const salePercent = 100 - signalSubModel.salePercent
        amount = (amount * salePercent) / 100
        roundedAmount = amount % 10000;//!new
        amount = amount - roundedAmount//!new
